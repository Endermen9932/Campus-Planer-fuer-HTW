...
/// Redirect-Following (damit `Set-Cookie` über Redirects hinweg – z.B. die
/// `JSESSIONID` nach dem Login-POST – zuverlässig übernommen wird).
///
/// Hinweis: nutzt `dart:io` und läuft daher auf Mobile/Desktop, **nicht** auf
/// Flutter Web. Web ist hier kein Zielsystem.
class IoLsfTransport implements LsfTransport {
  ...
  Future<LsfResponse> _send(
    ...
    try {
      ...
      // Redirects manuell verfolgen, damit Cookies erhalten bleiben.
      if (_isRedirect(response.statusCode) && redirectCount < maxRedirects) {
        final location = response.headers.value(HttpHeaders.locationHeader);
        await response.drain<void>();
        if (location != null) {
          final next = url.resolve(location);
          // 303 sowie 301/302 nach POST → GET; 307/308 behalten Methode.
          final keepMethod =
